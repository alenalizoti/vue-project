   ...
   <div class="parent">
        <form >
            ...
            <p>
                ...
                <input type="text" v-model="user.trenutno_stanje_novca">   
            </p>
            <p>
              <label>Vrsta korisnika</label>
              <select v-model="user.tip_korisnika">
            ...
            <p>
                ...
                <button class="btn btn-danger"><router-link to="/admin/users">Cancel</router-link></button>
            </p>
        </form>
    </div>
</template>
    ...
    methods :{
      fetchUser(){
       ...
       handleFileChange(){
        ...
        updateKorisnika(){
          axios.put(`http://127.0.0.1:5000/admin/users/update/${this.username}`,{
              username : this.user.username,
              password : this.user.password,
              email : this.user.email,
              godiste : this.user.godiste,
              putanja_img : this.putanja,
              stanje_novca : this.user.trenutno_stanje_novca,
              tip_korisnika : this.user.tip_korisnika
          })
          .then((response) => {
              console.log(response);
              this.provera = ''
